...
> 截图保存在 `docs/screenshots/` 目录，按以下清单截取。

| 序号 | 文件名 | 截图内容 | 对应评分维度 |
|------|--------|----------|------------|
| 1 | `01-主界面.png` | 游戏刚启动的完整窗口：15×15 棋盘 + 星位天元 + 右侧"状态/重新开始/悔棋"面板 | 核心功能 |
| 2 | `02-对战中.png` | 黑白双方各落多子、棋局进行中的画面，展示棋子和棋盘状态 | 核心功能 |
| 3 | `03-游戏结束.png` | 一方五连获胜，界面显示"X棋获胜!"的红色提示文字 | 胜负逻辑 |
| 4 | `04-代码结构.png` | IDE 或文件资源管理器中 `src/` 目录的 5 个模块文件，展示代码模块化架构 | 代码质量 |
| 5 | `05-算法核心.png` | `ai.py` 中 Negamax 搜索 + Alpha-Beta 剪枝的关键代码段截图 | 知识点应用 |
